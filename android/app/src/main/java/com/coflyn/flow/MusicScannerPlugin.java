...
public class MusicScannerPlugin extends Plugin {
    ...
    public MusicScannerPlugin() {
        Log.d("MusicScanner", "MusicScannerPlugin initialized");
    }
    ...
    public void chooseFolder(PluginCall call) {
        Log.d("MusicScanner", "chooseFolder called");
    ...
    @ActivityCallback
    public void chooseFolderResult(PluginCall call, ActivityResult result) {
        if (call == null) return;
    ...
    @PluginMethod
    public void pickLrcFile(PluginCall call) {
        Intent intent = new Intent(Intent.ACTION_OPEN_DOCUMENT);
        intent.addCategory(Intent.CATEGORY_OPENABLE);
        intent.setType("*/*");
        startActivityForResult(call, intent, "pickLrcFileResult");
    }

    @ActivityCallback
    public void pickLrcFileResult(PluginCall call, ActivityResult result) {
        if (call == null) return;
        if (result.getResultCode() == Activity.RESULT_OK) {
            Intent data = result.getData();
            if (data != null) {
                ...
                try {
                    ...
                    call.reject("Failed to read LRC file: " + e.getMessage());
                }
            } else {
